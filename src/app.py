...
activities = {
    ...
    "Basketball Club": {
        "description": "Pickup games, drills, and seasonal intramural play",
    ...
    "Robotics Club": {
        ...
        "schedule": "Tuesdays, 4:00 PM - 6:30 PM",
        "max_participants": 12,
        "participants": ["henry@mergington.edu", "ella@mergington.edu"]
    }
}

# add a simple lock to prevent race conditions
signup_lock = Lock()
...
def signup_for_activity(activity_name: str, email: EmailStr = Body(...)):
...
async def unregister_from_activity(activity_name: str, request: Request):
    ...
    Accepts either a JSON body like {"email": "student@..."} or a query parameter ?email=...
    """
    # Try to extract email from JSON body (if any)
    email = None
    try:
